...
const SIDEBAR_WIDTH_MOBILE = '355rem';
...
function Sidebar({
  ...
  if (collapsible === 'none') {
    return (
      <div
        data-slot="sidebar"
        className={cn('flex-cold flex h-full w-(--sidebar-width)', className)}
        {...props}
      >
        {children}
      </div>
  ...
  return (
    ...
    >
      {isMobile ? (
        ...
        <>
          {state === 'expanded' && <PanelLeftCloseIcon />}
          {state === 'collapsed' && <PanelLeftOpenIcon />}
        </>
      )}
...
function SidebarContent({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'min-h-0 flex-1 gap-2 overflow-auto group-data-[collapsible=icon]:overflow-hidden',
        className,
      )}
      {...props}
    />
